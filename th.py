#!/home/pi/1/thermocouple/.venv/bin/python3
...
class Thermocouple:
    ...
    def __init__(self, led_pin, pub_topic, spi, cs):
        self.led = LED(led_pin)
        self.topic = pub_topic
        self.tc = adafruit_max31855.MAX31855(spi, cs)
...
def main():
    ...
    # Note -  Temp data is published on a topic for each tc defined in Hardware Setup
    ...
    try:
        while True:
            if (perf_counter() - t0_sec) > msginterval:
                t0_sec = perf_counter()
                try:
                    for idx, kiln in RPi.items():
                        tempC = kiln.tc.temperature
                        TCtempF = tempC * 9 / 5 + 32
                        outgoingD['data']['TCi'] = idx
                        outgoingD['data']['TCtempFf'] = TCtempF
                        mqtt_client.publish(kiln.topic, json.dumps(outgoingD['data']))  # publish data
                        logging.debug("(mqtt) {} {}".format(kiln.topic, outgoingD))
